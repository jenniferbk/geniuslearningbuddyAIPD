// ContentAwareChatDemo.js - Updated with real transcript loading
...
const ContentAwareChatDemo = () => {
  ...
  const [viewMode, setViewMode] = useState('demo'); // 'demo', 'content', 'chat'
  const [transcriptStatus, setTranscriptStatus] = useState('unknown'); // 'unknown', 'loading', 'loaded', 'error', 'fetching'
  ...
  const handleContentContextChange = (context) => {
    console.log('🎯 ContentAwareChatDemo received context:', context);
  ...
  const handleProgressUpdate = (progressData) => {
    console.log('Content progress:', progressData);
  };

  // Debug user and auth state
  useEffect(() => {
    console.log('🔍 ContentAwareChatDemo Debug:', {
      user: user,
      userId: user?.id,
  ...
  const populateTranscript = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('✅ Transcript populated:', data);
        setTranscriptStatus('loaded');
      ...
      <div className="compact-header">
        <div className="course-info">
          ...
          {contentContext?.chunk?.topic && (
            <span className="current-section">• {contentContext.chunk.topic}</span>
          )}
              ...
              <div className="question-row">
                ...
                {contentContext?.chunk?.topic && (
                  <button 
                    ...
                    onClick={() => {
                      const event = new CustomEvent('insertSampleQuestion', {
                        detail: `Can you explain more about ${contentContext.chunk.topic}?`
                      });
                      window.dispatchEvent(event);
                    }}
                  >
                    Explain: {contentContext.chunk.topic.substring(0, 20)}...
                  </button>
                )}
              </div>
            </div>
